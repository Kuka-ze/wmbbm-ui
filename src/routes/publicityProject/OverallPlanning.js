...
import { Form, Card, Button, message, Select, Icon, Steps, TimePicker, DatePicker, Input } from 'antd';
...
function Index(props) {
  let { dispatch, form, current, address, map, templateName, show, steps, disabledCheck, detail = {}, id, team, user, userCount, selectedRows = [], selectedRowKeys, teamList = [] } = props;
  ...
  let formListProps = {
    ...
    onChangePage(pageNum) {
      dispatch({
        type: 'publicityOverallModel/ajaxList',
    ...
    selected(selectedRowKeys, selectedRows) {
      ...
      dispatch({
        type: 'publicityOverallModel/concat',
        payload: {
          ...props.params,
  ...
  function submitPool() {
    ...
    validateFields((err, values) => {
      ...
      /** 新增 */
      dispatch({
        type: 'publicityOverallModel/planAdd',
        payload: data
      });
      // nextStep()
    });
  ...
  function continuePool() {
    ...
    dispatch({
      type: 'publicityOverallModel/planInfo',
      payload: {
        id
      }
    })
  ...
  function backPool() {
    ...
    window.location.href = `#/publicityProject`
    ...
    <div>
      <div>
        ...
        <Card className="mt1">
          ...
          {current == 1 ? <div>
            <Map type={show} data={'杭州市|一号线'} handleMap={handleMap} />
          ...
          </div> : ''}
          {
            current == 2 ? <div className="box-bottom">
              ...
              <div className="detailed">可在「理论宣讲-活动池」查看该项宣讲活动的开展情况</div>
...
function mapStateToProps(state) {
  return {
    ...state.publicityOverallModel,
    loading: state.loading.models.publicityOverallModel
  };
}
