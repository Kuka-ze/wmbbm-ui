...
function IndexAddEdit(props) {
  ...
  function handleSubmit(e) {
    validateFields((err, values) => {
      ...
      if (props.id) {
        /** 编辑 */
        dispatch({
          type: 'publicityAddEditModel/ajaxEdit',
          payload: { ...data, id: props.id }
        });
      } else {
        /** 新增 */
        dispatch({
          type: 'publicityAddEditModel/ajaxAdd',
          payload: data
        });
      }
  ...
  function handleClick(e) {
    dispatch({
      type: 'publicityAddEditModel/concat',
      payload: {
        show: Math.random(),
      }
    });
              ...
              <Col span={24}>
                <Form.Item label="关联配置" {...formItemLayout2}>
                  {getFieldDecorator(`teamId[${k}]`, {
                    ...
                    rules: [{ required: true, message: "请选择可提供该项宣讲项目的队伍" }]
                  })(
                    <Select placeholder="请选择可提供该项宣讲项目的队伍" onChange={handleSelectChange}
                      showSearch
                      filterOption={false}
    ...
    <div>
      ...
      <Card>
        <Form>
          ...
          <Form.Item label="宣讲标题" {...formItemLayout}>
            {getFieldDecorator('templateName', {
              initialValue: detail.templateName,
              rules: [{ required: true, message: '请输入宣讲标题!', whitespace: true }],
            })(
              <Input type="text" placeholder="请输入宣讲标题" maxLength={50} />
            )}
          </Form.Item>
          <Form.Item label="宣讲类型" {...formItemLayout}>
            {getFieldDecorator('type', {
              initialValue: detail.type,
              rules: [{ required: true, message: '请选择宣讲类型!', whitespace: true }],
            })(
              <Select placeholder="请选择宣讲类型" >
          ...
          <Form.Item label="宣讲封面图" {...formItemLayout}>
            {getFieldDecorator('task_image', {
              initialValue: detail.task_image,
              rules: [{ required: true, message: '请上传宣讲封面图!' }],
            })(
          ...
          <Form.Item label="联系人" {...formItemLayout}
            ...
            <p>请输入该项宣讲的联系人联系方式，供群众或志愿者联系询问相关事宜。</p>
          </Form.Item>
...
function mapStateToProps(state) {
  return {
    ...state.publicityAddEditModel,
    loading: state.loading.models.publicityAddEditModel
  };
}
