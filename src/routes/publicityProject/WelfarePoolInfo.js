...
function Index(props) {
  ...
  function closeInfo() {
    confirm({
      title: '是否确认关闭公益',
      content: '关闭状态下群众将无法获取该宣讲,支持再次重启',
      okText: '确认关闭',
      cancelText: '取消',
      okType: 'danger',
      onOk() {
        dispatch({
          type: "PropagandaModel/updateAlipay",
          payload: {
            id,
            isAlipay: 2
          }
  ...
  function searchTable(e) {
    let query = e
    dispatch({
      type: "PropagandaModel/welfareInfo",
      payload: {
        ...props.params,
        state: query || ''
      }
        ...
        <Card>
          <Row type="flex" justify="space-between" style={styles.infoBox}>
            ...
            <Col style={{ textAlign: 'right' }}>
              ...
              <Link className="ml1" to={`/addPublicity?id=${id}`}>
                <Button>编辑</Button>
              </Link>
...
function mapStateToProps(state) {
  return {
    ...state.PropagandaModel,
    loading: state.loading.models.PropagandaModel
  };
}
