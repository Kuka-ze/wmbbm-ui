...
function IndexDetail(props) {
  ...
  function handleTab(key){
    if (key=='1') {
      dispatch({ type: 'StationManagmentDetailModel/concat', payload: { ...params, tab:"1" } });
      dispatch({ type: 'StationManagmentDetailModel/ajaxList', payload: { ...params, levelId, levelType, pageNum: 1 } });
    }else if (key=='2') {
      dispatch({ type: 'StationManagmentDetailModel/concat', payload: { ...params, tab:"2" } });
      dispatch({ type: 'StationManagmentDetailModel/ajaxList2', payload: { ...params, levelId, levelType, pageNum: 1 } });
    }
  }
  ...
  let tableProps = {
    ...
    pagination: {
      ...
      onChange(pageNum) {  
        dispatch({
          type: 'StationManagmentDetailModel/ajaxList',
  ...
  let tableProps2 = {
    ...
    pagination: {
      ...
      onChange(pageNum) {  
        dispatch({
          type: 'StationManagmentDetailModel/ajaxList2',
          payload: { ...props.params, pageNum }
        })
      }
    },
  ...
  return(
    <div>
      ...
      <Tabs activeKey={tab} onChange={handleTab} style={{background:'#fff', padding: '24px'}}>
        {paginationTotal>0?<TabPane tab="文明实践点" key="1">
          <Table {...tableProps} />
...
function mapStateToProps(state) {
  return {
    ...state.StationManagmentDetailModel,
    loading: state.loading.models.StationManagmentDetailModel
  };
}
