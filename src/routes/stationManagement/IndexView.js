...
function IndexView(props) {
  ...
  function handleSubmit(e) {
    validateFields((err, values) => {
      ...
      delete data.user;
      dispatch({
        type: 'StationManagementViewModel/ajaxAdd',
        payload: data
      });
    });
  }
  ...
  function handleClick(e) {
    dispatch({
      type: 'StationManagementViewModel/concat',
      payload: {
        show: Math.random(),
      }
    });
...
previewImage: ''
  ...
  function onSelectChange(num, value) {
    if (num == 1) {
      dispatch({
        type: 'StationManagementViewModel/ajaxPlaceList',
        payload: {
          centerId: value
        }
      });
...
function mapStateToProps(state) {
  return {
    ...state.StationManagementViewModel,
    loading: state.loading.models.StationManagementViewModel
  };
}
