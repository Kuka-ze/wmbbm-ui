...
function IndexAddEdit(props) {
  ...
  function handleSubmit(e) {
    validateFields((err, values) => {
      ...
      if(props.id){
        /** 编辑 */
        dispatch({
          type: 'PointManagementAddEditModel/ajaxEdit',
          payload: {...data, id: props.id}
        });
      }else{
        /** 新增 */
        dispatch({
          type: 'PointManagementAddEditModel/ajaxAdd',
          payload: data
        });
      }
    });
  ...
  function handleClick(e) {
    dispatch({
      type: 'PointManagementAddEditModel/concat',
      payload: {
        show: Math.random(),
      }
    });
  ...
  function onSelectChange(num, value) {
    if (num == 1) {
      dispatch({
        type: 'PointManagementAddEditModel/ajaxPlaceList',
        payload: {
          centerId: value,
          endLevel: 2
        }
      ...
      dispatch({
        type: 'PointManagementAddEditModel/ajaxStationList',
        payload: {
          placeId: value,
          endLevel: 1
        }
        ...
        <Form>
          ...
          <Form.Item label="点名称" {...formItemLayout}>
            {getFieldDecorator('siteName', {
              ...
              <Input type="text" placeholder="请输入点名称" maxLength={20} />
            )}
          </Form.Item>
          ...
          <Form.Item label="所属站" {...formItemLayout}>
            {getFieldDecorator('stationId', {
              ...
              <Select placeholder="请选择所属站"
                disabled={props.id ? true : false}
              >
                {stationList && stationList.length > 0 ? stationList.map((item, index) => <Select.Option value={item.key} key={index}>{item.value}</Select.Option>) : ''}
              </Select>
            )}
          </Form.Item>
          ...
          <Form.Item label="点介绍" {...formItemLayout}>
            {getFieldDecorator('content', {
              ...
              <TextArea rows={4} placeholder="请输入点介绍" maxLength="1000" />
            )}
          </Form.Item>
          <Form.Item label="类型" {...formItemLayout}>
            {getFieldDecorator('type', {
              ...
              >
                {siteType && siteType.length > 0 ? siteType.map((item, index) => <Select.Option value={item.key} key={index}>{item.value}</Select.Option>) : ''}
              </Select>
            )}
          </Form.Item>
          {formItems}
          ...
          <Form.Item label="点位置" {...formItemLayout}>
            {getFieldDecorator('address', {
              ...
              rules: [{ required: false, message: '请获取点位置!' }],
            })(
...
function mapStateToProps(state) {
  return {
    ...state.PointManagementAddEditModel,
    loading: state.loading.models.PointManagementAddEditModel
  };
}
