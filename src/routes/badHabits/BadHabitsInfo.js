...
function BadHabitsInfo(props) {
  ...
  function handlePreview(file) {
    dispatch({
      type: 'BadHabitsInfoModel/concat',
      payload: {
  ...
  return (
    <div>
      <Breadcrumb separator=">">
        ...
        <Breadcrumb.Item><Link to="/eventCenter">拍陋习</Link></Breadcrumb.Item>
        <Breadcrumb.Item>详情</Breadcrumb.Item>
      </Breadcrumb>
      <Form {...formItemLayout} style={{ backgroundColor: 'white', padding: '20px 0' }}>
        <Form.Item label="主题" style={{ marginBottom: '0px' }}>
          <span className="ant-form-text">{info.habitsName || '-'}</span>
        </Form.Item>
        <Form.Item label="描述" style={{ marginBottom: '0px' }}>
          <span className="ant-form-text">{info.habitsRemark || '-'}</span>
        </Form.Item>
        <Form.Item label="照片" >
        ...
        <Form.Item label="事件地点" style={{ marginBottom: '0px' }}>
          <span className="ant-form-text">{info.habitsAddress || '-'}</span>
        </Form.Item>
...
function mapStateToProps(state) {
  return {
    ...state.BadHabitsInfoModel,
    loading: state.loading.models.BadHabitsInfoModel,
  }
}

export default connect(mapStateToProps)(BadHabitsInfo);