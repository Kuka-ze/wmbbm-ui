...
function Index(props) {
  ...
  let formListProps = {
    ...
    onChangePage(pageNum) {
      dispatch({
        type: 'ActivityPoolModel/ajaxList',
    ...
    onUpdate(record, type) {
      if (type == 1) {
        dispatch({
          type: 'ActivityPoolModel/ajaxInfoService',
          payload: {
            id: record.id,
          },
        });
        dispatch({
          type: 'ActivityPoolModel/concat',
          payload: {
            visible: true,
            type,
        ...
        dispatch({
          type: 'ActivityPoolModel/ajaxVolunteerList',
          payload: {
            activityId: record.id,
          },
        });
        ...
        dispatch({
          type: 'ActivityPoolModel/ajaxCancel',
          payload: {
            activityId: record.id,
          },
        });
  ...
  function handleOk() {
    ...
    if (massesArr && massesArr.length > 0) {
      dispatch({
        type: 'ActivityPoolModel/ajaxMassesAdd',
        payload: {
          massesArr,
          activityId
        },
  ...
  function onIssueRejected(auditState) {
    ...
    dispatch({
      type: 'ActivityPoolModel/ajaxAudit',
      payload,
      callback: () => {
        dispatch({
          type: 'ActivityPoolModel/concat',
...
function mapStateToProps(state) {
  return {
    ...state.ActivityPoolModel,
    loading: state.loading.models.ActivityPoolModel
  };
}
