    ...
    columns: [
      {
        ...
        key: 'templateName',
        render: (text, record) => {
          let link1 = `/activityPoolView?id=${record.id}`;
          return <div>
            <Link to={link1} onClick={() => setCacheData(params)}>{text ? text : '-'}</Link>
          </div>
        },
      }, {
        title: '群众',
      ...
      },
      {
        ...
        render: (text, record) => {
          ...
          return <p><Badge color={activityState == 1 ? "#F59A23" : activityState == 2 ? "#0E77D1" : activityState == 3 ? "#00A854" : activityState == 4 ? "#D7D7D7" : "#d9d9d9"} />{text}</p>
        }
      },
      {
        title: '活动时间',
