...
export default {
  ...
  effects: {
    *init({ payload }, { call, put, select }) {
      const { templateId } = yield select(state => state.ActivityPoolModel);
      const states = {
        params: {
          pageNum: 1,
          pageSize: 10,
    ...
    *ajaxCancel({ payload }, { call, put, select }) {
      ...
      if (code === 1) {
        ...
        const { params, paginationTotal } = yield select(state => state.ActivityPoolModel);
        yield put({
          type: 'ajaxList',
