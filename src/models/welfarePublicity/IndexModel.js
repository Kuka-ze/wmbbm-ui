...
export default {
  namespace: 'WelfarePublicityModel',
  state: {},
  reducers: {
  ...
  effects: {
    ...
    *ajaxAreaTree({ payload }, { call, put, select }) {
    ...
    /** 置顶 */
    *welfareTop({ payload }, { call, put, select }) {
      const { code } = yield call(PublicService.welfareTop, payload);
      if (code == 1) {
        message.success('操作成功');
        const { params, paginationTotal } = yield select(state => state.WelfarePublicityModel);
        yield put({
          type: 'ajaxList',
    ...
    *ajaxIsAdministrators({ payload }, { call, put, select }) {
      ...
      if (code == 1) {
        if (data.administratorsv == 1) {
          window.location.href = '#welfarePublicityAdd';
        } else {
