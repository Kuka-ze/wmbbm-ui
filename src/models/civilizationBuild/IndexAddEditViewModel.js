...
export default {
  namespace: 'CivilizationBuildAddEditViewModel',
  state: {},
  reducers: {
  ...
  effects: {
    ...
    *ajaxEditDetail({ payload }, { call, put, select }) {
      ...
      if (code === 1) {
        if (data.buildImg && data.buildImg.length > 0) {
          let imgArr = [];
          data.buildImg.map((item, index) => {
            let obj = {
              uid: index,
              name: `img0`,
              status: 'done',
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname, search }) => {
        ...
        if (pathname === '/civilizationBuildAdd') {
          initData();
        } else if (pathname === '/civilizationBuildEdit') {
          initData();
          if (query.id) {
            dispatch({
              type: 'concat', payload: {
        ...
        } else if (pathname === '/civilizationBuildView') {
          initData();
          if (query.id) {
            dispatch({
              type: 'concat', payload: {
