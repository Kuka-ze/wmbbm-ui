...
import PublicService from '../../services/CivilizationBuild';
import { getCacheData, removeCacheData } from '../../utils/util';
export default {
  namespace: 'CivilizationBuildModel',
  state: {},
  reducers: {
  ...
  effects: {
    ...
    *noticeTypeDrop({ payload }, { call, put, select }) {
    ...
    /** 置顶 */
    *buildTop({ payload }, { call, put, select }) {
      const { code } = yield call(PublicService.buildTop, payload);
      if (code == 1) {
        message.success('操作成功');
        const { params, paginationTotal } = yield select(state => state.CivilizationBuildModel);
        yield put({
          type: 'ajaxList',
    ...
    *ajaxIsAdministrators({ payload }, { call, put, select }) {
      ...
      if (code == 1) {
        if (data.administratorsv == 1) {
          window.location.href = '#civilizationBuildAdd';
        } else {
