...
export default {
  namespace: 'CentralManagementDetailModel',
  state: {},
  reducers: {
  ...
  effects: {
    ...
    *ajaxList2({ payload }, { call, put, select }) {
      const { data, code } = yield call(PublicService.ajaxTabList2, payload);
      if (code === 1) {
        let { list, totalSize } = data;
        yield put({
          type: 'concat',
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname, search }) => {
        ...
        if (pathname === '/centralManagementDetail') {
          initData();
          dispatch({ type: 'ajaxList', payload: { ...params, levelId, levelType: 1 } });
          dispatch({ type: 'ajaxList2', payload: { ...params, levelId, levelType: 1 } });
        }
      })
    }
  }
