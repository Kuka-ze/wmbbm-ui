...
export default {
  namespace: 'NoticePublicityModel',
  state: {},
  reducers: {
  ...
  effects: {
    ...
    *noticeTypeDrop({ payload }, { call, put, select }) {
    ...
    /** 置顶 */
    *noticeTop({ payload }, { call, put, select }) {
      const { code } = yield call(PublicService.noticeTop, payload);
      if (code == 1) {
        message.success('操作成功');
        const { params, paginationTotal } = yield select(state => state.NoticePublicityModel);
        yield put({
          type: 'ajaxList',
    ...
    *ajaxIsAdministrators({ payload }, { call, put, select }) {
      ...
      if (code == 1) {
        if (data.administratorsv == 1) {
          window.location.href = '#noticePublicityAdd';
        } else {
