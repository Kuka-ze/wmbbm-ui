...
import PublicService from '../../services/EventCenter';
import { getCacheData, removeCacheData } from '../../utils/util';
export default {
  namespace: 'EventCenterModel',
  state: {},
  reducers: {
  ...
  effects: {
    ...
    *ajaxDelete({ payload }, { call, put, select }) {
      ...
      if (code === 1) {
        ...
        const { params, paginationTotal } = yield select(state => state.EventCenterModel);
        yield put({
          type: 'ajaxList',
