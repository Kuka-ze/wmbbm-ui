...
let map, cityName, geolocation, geocoder, marker;
class Map extends React.Component {
  constructor(props) {
    ...
    this.regeoCode = this.regeoCode.bind(this);
  }
  ...
  regeoCode() {
    ...
    geocoder.getAddress(lnglats, function (status, result) {
      if (status === 'complete' && result.regeocode) {
        let address = result.regeocode.addressComponent.province + result.regeocode.addressComponent.city + result.regeocode.addressComponent.district + result.regeocode.addressComponent.township + result.regeocode.addressComponent.street + result.regeocode.addressComponent.streetNumber;
        that.setState({
          location_name: address
        });
      } else {
        message.error('根据经纬度查询地址失败', 2);
      }
    });
  }
  nameSearch(keyword) {
        ...
        <div className="page1-component">
          <Form layout="inline" >
            <FormItem {...formItemLayout} label="搜索">
              <SearchInput placeholder="请输入地址" onSearch={this.nameSearch.bind(this)} keyword={this.state.keyword} handleInputChange={this.handleInputChange.bind(this)}/>
            </FormItem>
            ...
            <div style={{ paddingTop: '10px', color: 'rgba(0, 0, 0, 0.85)'}}>详细地址: {this.state.location_name ? this.state.location_name : '-'}</div>
          </Form>
          <p>
            {
              //   list.map( (item, index ) => {
