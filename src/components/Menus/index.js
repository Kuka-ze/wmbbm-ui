...
class Menus extends React.Component {
  ...
  handleChange(n ,e) {
    ...
    if (n.lastIndexOf('-') > -1) {
      ...
      if(d){
        menuChecked[d] = e.target.checked ? true : false;
      }
      let oneChecked = false;
      let twoChecked = false;
      let threeChecked = false;
      //console.log(a,"arr")
      if (arr[3] == undefined){
        //二级选中
        twoChecked = e.target.checked ? true : false;
        menuChecked[b] = twoChecked ? true : false;
        threeChecked = twoChecked ? true : false;
        menus.map(vvv => {
          if (vvv.id == a) {
            if (Array.isArray(vvv.children)) {
              vvv.children.map(vv => {
                  ...
                  if (Array.isArray(vv.children)) {
                    vv.children.map(v => {
                      ...
                      if(v.id == c || !arr[2]){
                        if (Array.isArray(v.children)) {
                          v.children.map(vs => {
                            menuChecked[vs.id] = e.target.checked ? true : false;  //操作按钮根据二级菜单的选中或取消
                          })
                        }
                      }
                    })
      ...
      console.log("aa:", oneChecked, twoChecked, threeChecked);
      menuChecked[a] = oneChecked ? true : false;
      menuChecked[b] = twoChecked ? true : false;
      if(menuChecked[c] != undefined){
        menuChecked[c] = threeChecked ? true : false;
      }
    } else {
      //点击一级菜单
        ...
        if(vvv.id == n) {
          if (Array.isArray(vvv.children)) {
            vvv.children.map(vv=>{
              ...
              if (Array.isArray(vv.children)) {
                vv.children.map(v => {
                  menuChecked[v.id] = e.target.checked ? true : false; //三级菜单跟着一级菜单选中或者取消
                  if (Array.isArray(v.children)) {
                    v.children.map(vs => {
                      menuChecked[vs.id] = e.target.checked ? true : false; //操作按钮跟着一级菜单选中或者取消
                    })
                  }
                })
              }
                    ...
                    Array.isArray(item.children) ? item.children.map((items,indexs)=>{
                      return (
                        <div key={`row-${item.id}-${indexs}`} style={(this.lookup(item.id,"1级"))}>
                          ...
                          {
                            Array.isArray(items.children) ? items.children.map((itemss,indexs)=>{
                              return (
                                <div key={`row-${item.id}-${indexs}`} style={(this.lookup(item.id,"1级"))}>
                                  <Row style={{marginLeft:60}} className="mt1 sanjiao_zhu" key={`rows-${index}`}>
                                    {
                                      this.lookup(itemss.id,"2级") ? <div className="sanjiao_shu_right" onClick={this.retract.bind(this,itemss.id,"2级")}></div> : <div className="sanjiao_shu_xia" onClick={this.retract.bind(this,itemss.id,"2级")}></div>
                                    }
                                    {getFieldDecorator(`menuItem-${item.id}-${items.id}-${itemss.id}`,{})(
                                      <Checkbox
                                        disabled = {this.props.disabled ? true : false}
                                        checked={this.state.menuChecked[itemss.id]}
                                        key={itemss.id}
                                        onChange={this.handleChange.bind(this, `${item.id}-${items.id}-${itemss.id}`)}>
                                        {itemss.name}</Checkbox>
                                    )}
                                  </Row>
                                  <Row style={{marginLeft:90}} key={`rows-${index}-${indexs}`}>
                                    <div style={(this.lookup(itemss.id,"2级"))}>
                                      {Array.isArray(itemss.children) ? itemss.children.map((itm,idx)=>{
                                        return getFieldDecorator(`btn-${item.id}-${itemss.id}-${itm.id}`,{})(
                                          <Checkbox
                                            disabled = {this.props.disabled ? true : false}
                                            checked={this.state.menuChecked[itm.id]}
                                            key={itm.id} onChange={this.handleChange.bind(this, `${item.id}-${items.id}-${itemss.id}-${itm.id}`)}>
                                            {itm.name}</Checkbox>
                                        )
                                      }) : "" }
                                    </div>
